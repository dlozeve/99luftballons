...
# 'validation' if testing the model, 'test' if using test data to
...
nTrainDays = 2
nValidationDays = 2
# The input forecast models to use for the prediction. Must have
# values between 0 and 9 (/!\ 10 is the true data)
...
if dataType == 'validation':
    ...
    x_train, x_val, y_train, y_val = handling.splitTrainVal(train, nTrainDays,
                                                            nValidationDays,
                                                            useModels)
elif dataType == 'test':
    print("Reshaping the data for prediction")
    x_train, _, y_train, _ = handling.splitTrainVal(train, nTrainDays, 0,
                                                    useModels)
    x_test, _, _, _ = handling.splitTrainVal(test, 5, 0, useModels)
del train, test

# Fit the estimator and perform the prediction. Evaluate graphically
# and with some metrics
if dataType == 'validation':
    pred_train, pred_val = learning.makePrediction(x_train, x_val, y_train,
                                                   pred_model, modelName, red)
    learning.evaluatePrediction(pred_train, pred_val, y_train, y_val,
                                nValidationDays, evalThreshold)
    predMatrix, trueMatrix = learning.formatPrediction(pred_val, y_val,
                                                       nValidationDays)
elif dataType == 'test':
    _, pred_test = learning.makePrediction(x_train, x_test, y_train,
                                           pred_model, modelName, red)
    predMatrix, _ = learning.formatPrediction(pred_test, None, 5)
    trueMatrix = None

# Use the prediction to perform the pathfinding and a few
# evaluations. Output one path for verification as well as a complete
# submission
...
submission = pfind.getSubmission(algo, citydata, predMatrix,
                                 nValidationDays, dataType, trueMatrix)
